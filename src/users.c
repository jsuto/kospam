...
int getUserdataFromEmail(struct session_data *sdata, char *email, struct __config *cfg){
   ...
   if(cfg->verbosity >= _LOG_DEBUG) syslog(LOG_PRIORITY, "%s: user filter: %s", sdata->ttmpfile, filter);
   ...
   if(e){
      ...
      if(ldap_count_values(vals) > 0){
         sdata->uid = atol(vals[0]);

         /* provide gid=uid in case the clapfgid attribute does not exist */
         sdata->gid = sdata->uid;
      }
      ldap_value_free(vals);

      vals = ldap_get_values(sdata->ldap, e, "clapfgid");
      if(ldap_count_values(vals) > 0) sdata->gid = atol(vals[0]);
      ldap_value_free(vals);
