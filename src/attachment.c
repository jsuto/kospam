...
bool has_known_bad_attachment(MYSQL *conn, struct session_data *sdata, struct Message *m) {
   ...
   for (int i=0; i<m->n_attachments; i++) {
      syslog(LOG_PRIORITY, "INFO: %s: name=*%s*, type=%s, size=%ld, digest=%s", sdata->ttmpfile, m->attachments[i].filename, m->attachments[i].type, m->attachments[i].size, m->attachments[i].digest);
      ...
      if (counter > 0) {
         ...
         syslog(LOG_PRIORITY, "INFO: %s: digest=%s is marked as malware (%llu)", sdata->ttmpfile, m->attachments[i].digest, counter);
      }
   }
