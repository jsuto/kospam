...
create index connection_idx on connection(queue_id, client, ts, `from`, ts);
...
create index smtp_idx on smtp(queue_id, clapf_id, `relay`, `to`, ts);
...
create index clapf_idx on clapf(queue_id2, `from`);
